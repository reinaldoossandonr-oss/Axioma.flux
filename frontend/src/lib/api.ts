...
const BUCKET_DISENOS_3D = 'disenos-3d'
const MIME_3D_PERMITIDOS = ['model/gltf-binary', 'model/gltf+json', 'application/octet-stream']
const TAMANO_MAXIMO_3D_BYTES = 50 * 1024 * 1024 // 50MB, igual que el límite del bucket

export const ubicacionesApi = {
  listar: () => apiFetch<any[]>('/ubicaciones'),
  crear: (data: any) => apiFetch<any>('/ubicaciones', { method: 'POST', body: JSON.stringify(data) }),
  actualizar: (id: string, data: any) =>
    apiFetch<any>(`/ubicaciones/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
   ...
   * Supabase Storage y persiste la URL resultante en ubicaciones.diseno_3d_url
   * vía el backend. Ruta del objeto: {empresa_id}/{ubicacion_id}.glb — la
   * política RLS del bucket solo permite escribir dentro de la carpeta de la
   * propia empresa. Los nombres de los objetos dentro del modelo 3D deben
   * coincidir exactamente con posiciones.codigo para el emparejamiento automático.
   */
  subirDiseno3D: async (ubicacionId: string, file: File): Promise<string> => {
    const esGlbPorExtension = /\.(glb|gltf)$/i.test(file.name)
    if (!MIME_3D_PERMITIDOS.includes(file.type) && !esGlbPorExtension) {
      throw new ApiError(400, 'Formato no permitido. Usa un archivo .glb o .gltf.')
    }
    if (file.size > TAMANO_MAXIMO_3D_BYTES) {
      throw new ApiError(400, 'El diseño 3D no puede superar los 50MB.')
    }
    ...
    const path = `${(perfil as any).empresa_id}/${ubicacionId}.${ext}`

    const { error: uploadError } = await supabase.storage
      .from(BUCKET_DISENOS_3D)
    ...
    const disenoUrl = `${publicUrlData.publicUrl}?v=${Date.now()}`

    await ubicacionesApi.actualizar(ubicacionId, { diseno_3d_url: disenoUrl })
