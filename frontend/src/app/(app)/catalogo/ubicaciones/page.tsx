    ...
    <div className="p-4 md:p-6 h-full flex flex-col gap-4 md:gap-5">
      <div className="flex items-center justify-between gap-3 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button
            onClick={() => setMostrarUbicaciones(v => !v)}
            aria-expanded={mostrarUbicaciones}
            className="flex items-center gap-2 text-sm whitespace-nowrap px-3 py-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors"
          >
            <IconChevronDown className={`w-4 h-4 flex-shrink-0 transition-transform ${mostrarUbicaciones ? '' : '-rotate-90'}`} />
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-4 md:gap-5">
        {/* Barra horizontal de ubicaciones: chips en vez de sidebar vertical, para no dejar espacio en blanco cuando hay pocas ubicaciones. Colapsable con el botón "Ubicaciones" del encabezado para liberar espacio para el visor 3D. */}
        {mostrarUbicaciones && (
          <div className="card flex-shrink-0">
...
function IndicadorBarra({
  ...
  return (
    <div className="flex items-center gap-1.5 px-2.5 py-1.5 min-w-0">
      ...
      <span className="text-sm font-bold text-slate-800 leading-none truncate">{value}</span>
      <span className="text-xs text-slate-400 leading-none truncate">{label}</span>
    </div>
  )
}
